...
//event occurs.
#define TRAP_TABLE_SIZE         (5)
#define TRAP_COMMUNITY_MAX_LEN       (16)
...
/****************************************************************************
  ...
  ***************************************************************************/
void SNMPInit(void);
void SpaSibSNMPInit(UDP_PORT ReadPort); // доработанная ф-я инициализации UDP сокета для SNMP read - можно менять порты
BOOL SNMPTask(void);
...
BOOL SNMPIsNotifyReady(IP_ADDR* remoteHost, UDP_PORT SnmpTrapPort);         // Доработали метод - теперь можновыбирать порт для отправки трапов
