...
// Наша инициалия UDP сокета для работы с SNMP (запросы)
// Вызывать всместо (или после) SNMPInit()
void SpaSibSNMPInit(UDP_PORT ReadPort)
{
    // Если SNMP сокет открыт и используется - убиваем
    if(SNMPAgentSocket != INVALID_UDP_SOCKET)
         UDPClose(SNMPAgentSocket);
    
    SNMPStatus.Val = 0;
    SNMPAgentSocket = UDPOpen(ReadPort, 0, INVALID_UDP_SOCKET); // Открываем новый сокет с заданным портом
}
 ...
 ***************************************************************************/
BOOL SNMPIsNotifyReady(IP_ADDR* remoteHost, UDP_PORT SnmpTrapPort)
{
    ...
    if ( ARPIsResolved(remHostIpAddrPtr, &remoteNode.MACAddr) )
    {
        ...
        SNMPNotifyInfo.socket = UDPOpen(AGENT_NOTIFY_PORT, &remoteNode, /*SNMP_NMS_PORT*/SnmpTrapPort);
